  ...
  <div :class="'chat-container my-2 ' + (isReply ? 'reply' : '')">
    ...
    <div class="chat-component" v-else>
      <div>
        ...
        <ChatOptions :show="currentUser == user.username" class="md:hidden" @reply="onReply" @edit="onEdit" @delete="onDelete" />
      </div>
      <div class="flex flex-col w-full">
        <div class="flex items-center justify-between">
          <div class="flex pb-2 items-center justify-center space-x-2">
            ...
            <span class="font-bold text-sm text-white bg-moderate-blue rounded-sm px-1.5" v-if="currentUser == user.username"> you </span>
            <span class="text-greyish-blue">{{ time }}</span>
          </div>
          <ChatOptions :show="currentUser == user.username" class="hidden md:flex" @reply="onReply" @edit="onEdit" @delete="onDelete" />
        </div>
        <p class="text-greyish-blue" v-if="!editing">
          <span class="font-medium cursor-pointer text-moderate-blue" v-if="repliedTo">
            {{ '@' + repliedTo }}
          </span>
          {{ content }}
        </p>
        <textarea @keyup.esc="editing = false" v-model="editedContent" @keyup.enter="updateContent" autofocus v-else></textarea>
        <button @click="updateContent" class="bg-moderate-blue rounded-md ml-auto mt-3 py-2 px-3 text-white uppercase font-medium" v-if="currentUser == user.username && editing">Update</button>
      </div>
    </div>
    <!-- <DeletePopup
      :show="showPopup"
...
export default {
  ...
  methods: {
    updateScore(score) {
      if (this.currentUser.username != this.user.username) this.$emit('score-changed', score, this.id);
    },
    onDelete() {
      this.showPopup = true;
    },
